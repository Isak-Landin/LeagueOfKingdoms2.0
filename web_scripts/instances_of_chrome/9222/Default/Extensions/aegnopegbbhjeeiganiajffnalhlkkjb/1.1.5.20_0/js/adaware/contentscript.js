...
(function () {
        ...
        chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
            ...
            if (request.what == "badgeClicked") {
                ...
                if (findPopupFrame === null) {
                    ...
                    document.getElementById("sts_permission_panel").remove();
                    sendResponse({ showPopup: false });
                }
            }

            if (request.text == "reloadPage") {
                sendResponse("Page reloaded!");
    ...
    window.addEventListener("DOMContentLoaded", (event) => {
        ...
        // remove torrent scanner popup on document clicked
        document.addEventListener("click", (e) => {
            

            if (e.target?.parentNode?.id !== "sts_permission_panel" && e.target?.parentNode?.id !== "sts_container" && e.target?.parentNode?.id !== "sts_promotion_panel") {
                if (document.getElementById("sts_frame_container")) {
                    document.getElementById("sts_frame_container").remove();
                }
                ...
                if (document.getElementById("sts_promotion_panel")) {
                    document.getElementById("sts_promotion_panel").remove();
                }
            }
        });
    });
